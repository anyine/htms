		...
		.define(
				'Ext.ux.RowExpander',
				{
					...
					constructor : function() {
						...
						// </debug>
						// TODO: if XTemplate/Template receives a template as an
						// arg, should
						// just return it back!
						var rowBodyTpl = Ext.create('Ext.XTemplate',
								this.rowBodyTpl), features = [ {
							ftype : 'rowbody',
					...
					init : function(grid) {
						...
						// headerCt). Otherwise, shared column configs get
						// corrupted, e.g., if put in the
						// prototype.
					...
					getRowBodyFeatureData : function(data, idx, record, orig) {
						var o = Ext.grid.feature.RowBody.prototype.getAdditionalData
								.apply(this, arguments), id = this.columnId;
					...
					onEnter : function(e) {
						var view = this.view, ds = view.store, sm = view
								.getSelectionModel(), sels = sm.getSelection(), ln = sels.length, i = 0, rowIdx;
					...
					toggleRow : function(rowIdx) {
						var rowNode = this.view.getNode(rowIdx), row = Ext
								.get(rowNode), nextBd = Ext.get(row).down(
								this.rowBodyTrSelector), record = this.view
								.getRecord(rowNode), grid = this.getCmp();
						...
						if (row.hasCls(this.rowCollapsedCls)) {
							...
							this.view.fireEvent('collapsebody', rowNode,
									record, nextBd.dom);
						}

						// If Grid is auto-heighting itself, then perform a
						// component layhout to accommodate the new height
					...
					getHeaderConfig : function() {
						var me = this, toggleRow = Ext.Function.bind(
								me.toggleRow, me), selectRowOnExpand = me.selectRowOnExpand;
						...
						return {
							...
							processEvent : function(type, view, cell,
									recordIndex, cellIndex, e) {
								if (type == "mousedown"
